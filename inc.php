	...
	include 'deff.php';
			...
			function MyNEWBase($dbPW) {
				...
				$query = "INSERT INTO notes (id, name, email, text) VALUES
					(2, '-', '-', '			<p>Работаю электромонтёром в электроцехе более 30 лет, увлекаюсь велосипедами, \r\n		постоянно пополняю и обновляю свою велоколлекцию.</p>\r\n		<p>Картинка на главной страничке моего сайта понравилась мне больше всего, поэтому я выбрал именно её.</p>\r\n		<p>В интернете есть множество ссылок, посвященных велосипедам, которые я регулярно посещаю и прочитываю.</p>\r\n		<p>Постоянно поддерживаю беседы про велосипеды, которые можно продолжить в гостевой книге моего сайта.</p>\r\n'),
					(3, '-', '-', '			<p>Это конечно же велоспорт и все с этим связанное. </p>\r\n			<p>Я регулярно смотрю спортивные события и болею за наших велоспортсменов. </p>\r\n			<p>Вообще, обажаю спорт! </p>\r\n			<p>Спорт прекрасен!</p>\r\n'),
			...
			function MySaveBDSQL() {
				...
				if (!$db)
					...
					if ($result) {
						...
						mysqli_select_db($db, $MyUserTable);	
						$result=mysqli_query($db, "SET NAMES utf8");						
						$query = "UPDATE notes SET text='".$_POST["msg1"]."' WHERE ID = 2";
						$result = mysqli_query($db,$query);
					
						$query = "UPDATE notes SET text='".$_POST["msg2"]."' WHERE ID = 3";
						$result = mysqli_query($db,$query);
					
						$query = "UPDATE notes SET text='".$_POST["msg3"]."' WHERE ID = 4";
						$result = mysqli_query($db,$query);

						$query = "UPDATE notes SET text='".$_POST["msg4"]."' WHERE ID = 5";
						$result = mysqli_query($db,$query);				
